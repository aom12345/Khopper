...
NUM_MC_SIMULATIONS = 500    # MC runs per scenario (Higher is safer now that it's fast)
NUM_OPPONENTS = 1
...
def run_simulation_with_full_stats():
    ...
    print(f"Starting Optimized Simulation: {NUM_OUTER_ITERATIONS} scenarios.")
    print("-" * 60)
    
    start_total = time.time()
    
    for i in range(NUM_OUTER_ITERATIONS):
        # 1. Generate Scenario (Hero + 4 Board Cards)
        # We need 6 unique cards total
        scenario_cards = random.sample(base_deck, 6)
        hero_hand = scenario_cards[0:2]
        turn_board = scenario_cards[2:6]
        
        # Identify remaining deck for MC
        ...
        # Cards needed per run: 1 River + (2 * Opponents)
        cards_needed = 1 + (NUM_OPPONENTS * 2)
        
        for _ in range(NUM_MC_SIMULATIONS):
            # Fast sampling
            drawn = random.sample(deck_remainder, cards_needed)
            river = drawn[0]
            opp_draws = drawn[1:]
            
            final_board = turn_board + [river]
            
            # Evaluate Hero
            hero_score = evaluate_hand_score_fast(hero_hand + final_board)
            
            # Evaluate Opponents
            best_opp_score = -1
            for k in range(0, len(opp_draws), 2):
                op_hand = [opp_draws[k], opp_draws[k+1]]
                s = evaluate_hand_score_fast(op_hand + final_board)
                if s > best_opp_score:
                    best_opp_score = s
...
# --- Reporting Functions (Original Format) ---
